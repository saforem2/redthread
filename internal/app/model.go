...
type model struct {
	...
	// history holds whole-board snapshots for undo/redo. Every mutation
	// goes through m.mutate, which pushes here before the change lands.
	history *History
}
...
// mutate is the single seam every board change passes through: it
// snapshots the board for undo, then schedules a debounced save. label
// names the change and appears in the toast when it is undone.
//
...
func (m model) handleBoardKey(key string) (tea.Model, tea.Cmd) {
	...
	case ">", ".":
		...
		m.refreshActive()
		m.saver.Touch() // board switch: not a content change, nothing to undo
	...
	case tea.MouseActionRelease:
		if m.grabID != "" {
			if n := m.findNote(m.grabID); n != nil {
				...
				if moved {
					// Drop bounce after an actual move. The undo snapshot
					// was taken at press time, above.
					n.Bob = 2.2
					n.Updated = time.Now().UTC()
					m.saver.Touch()
				} else {
